window.COURSE_DATA = {
  ...
  modules: [
    ...
    { id: "M4", number: "04", title: "动力学与轨迹规划", hours: "10学时", level: "分析 / 创造", color: "orange", prereq: "M3", problem: "轨迹看起来平滑，是否就满足关节限位、速度和动力学边界？", task: "在MATLAB生成六轴五次轨迹，用同一q(t)同步驱动ABB三维参考关节链；先预测动作，再比较曲线、构型与约束。", evidence: ["插值公式与代码", "六轴曲线—三维动作对应记录", "约束检查", "奇异反例"], rubric: "公式—代码一致；曲线与动作可对应；全部约束有数值；反例能复现。", ai: "可指出哪类约束未检查；不得替学生完成预测、复算或得出安全结论。", resource: "./resources/textbook/ch06.html", nodes: ["动力学", "五次插值", "同源同步可视化", "速度约束", "奇异筛查"] },
    { id: "M5", number: "05", title: "控制、感知与虚实校核", hours: "10学时", level: "应用 / 评价", color: "green", prereq: "M3,M4", problem: "教学用三维参考模型与RobotStudio、控制器和实体机器人之间有哪些证据边界？", task: "完成单关节控制实验，分析采样、误差、噪声和执行约束，并区分同步可视化、控制器验证与实体安全放行。", evidence: ["控制指标", "响应曲线", "虚实边界表", "安全放行清单"], rubric: "指标定义清楚；三维可视化不冒充实体结果；异常可解释。", ai: "辅助阅读曲线和定位异常，参数整定与结论由学生完成。", resource: "./resources/textbook/ch07.html", nodes: ["PID控制", "传感器", "虚实差异", "结果责任"] },
  ...
  compliance: [
    ...
    { item: "常态化智慧教学场景", implementation: "入口诊断、三级提示、MATLAB轨迹—ABB三维动作同步、证据档案。", status: "已实现" },
    { item: "多类型数字资源", implementation: "迁移8章数字教材、交互动画与可复算工程代码。", status: "已实现" },
    { item: "数据驱动评价与改进", implementation: "知识理解40%、任务证据40%、无AI迁移20%形成掌握诊断；正式成绩仍须教师核证。", status: "已实现原型" },
  ...
  aiPolicy: {
    levels: { 1: "定位问题", 2: "提供检查路径", 3: "学生尝试后提供局部支架" },
    moduleRules: {
      M1: { maxLevel: 3, stage: "充分支架", reason: "建立参数检索与证据边界习惯" },
      M2: { maxLevel: 3, stage: "充分支架", reason: "形成坐标系与矩阵检查程序" },
      M3: { maxLevel: 3, stage: "尝试后支架", reason: "三级提示仅在完成一次尝试后开放" },
      M4: { maxLevel: 2, stage: "减少支架", reason: "只给约束检查路径，不给局部计算结果" },
      M5: { maxLevel: 1, stage: "诊断提示", reason: "只定位虚实差异类别" },
      M6: { maxLevel: 0, stage: "AI退出", reason: "综合项目与迁移测评独立完成" }
    },
    evidenceThreshold: 80,
    humanReview: "AI反馈只用于形成性评价；正式达成度、RobotStudio验证和实体安全由教师与实验流程复核。"
  },
  abb: {
    model: "ABB IRB 1200-5/0.9",
    jointLimits: [[-170,170],[-100,130],[-200,70],[-270,270],[-130,130],[-400,400]],
    maxSpeed: [288,240,297,400,405,600],
    speedContext: "ABB产品规范中的三相供电最大轴速度；单相供电时A4/A5上限降低为376/399 °/s，实体任务以控制器配置为准。",
    ...
    source: "ABB产品规范3HAC081417-001 Revision M（工作范围、三相/单相速度）；ROS-Industrial abb_irb1200_support参考链（教学运动链）"
  }
};
